  ...
  "node_modules/react/cjs/react.development.js"(exports, module) {
    ...
    "production" !== process.env.NODE_ENV && function() {
      ...
      function elementRefGetterWithDeprecationWarning() {
        var componentName3 = getComponentNameFromType(this.type);
        didWarnAboutElementRef[componentName3] || (didWarnAboutElementRef[componentName3] = true, console.error(
          "Accessing element.ref was removed in React 19. ref is now a regular prop. It will be removed from the JSX Element type in a future release."
        ));
        componentName3 = this.props.ref;
        return void 0 !== componentName3 ? componentName3 : null;
      }
...
var handleTargets2 = (handle) => [
  ...
  ...handle.connectedEdges.map((edge) => edgeTarget2(edge, [handle.node]))
];
var netTargets2 = (net) => {
  const nodes = new Map(net.handles.map((handle) => [handle.node.id, nodeTarget2(handle.node)]));
  const netNodes = net.handles.map((handle) => handle.node);
  const edges = new Map(net.edges.map((edge) => [edge.id, edgeTarget2(edge, netNodes)]));
  return [...nodes.values(), ...edges.values()];
};
...
var isDigitalSinkCapable = (handle) => digitalSinkFunctions.some((fn) => hasFunction3(handle, fn));
var isDigitalSink = isDigitalSinkCapable;
var isDigitalSourceCapable = (handle) => digitalSourceFunctions.some((fn) => hasFunction3(handle, fn));
...
var hasSignalSourceCapability = (handle, definition) => definition.sourceFunctions.some((fn) => hasFunction3(handle, fn));
var hasSignalSinkCapability = (handle, definition) => definition.sinkFunctions.some((fn) => hasFunction3(handle, fn));
...
var checkDuplicateParallelWires = (context) => {
  ...
  return Array.from(groups.values()).filter((edges) => edges.length > 1).map((edges) => translatedIssue2(
    ...
    "info",
    { count: edges.length },
    edges.map((edge) => edgeTarget2(edge)),
...
var checkMainsWireConnectedToLowVoltageComponent = (context) => context.componentLinkedNets.filter((net) => netHasAnyClassification(net, ["L_net_type", "N_net_type", "PE_net_type"])).flatMap((net) => net.handles.filter((handle) => !hasMainsFunction(handle)).filter((handle) => isTechnicalComponent(handle.node)).filter((handle) => !isPassiveConnectorComponent(handle.node)).map((handle) => ({ net, handle }))).map(({ net, handle }) => translatedIssue2(
  ...
  "error",
  { component: componentName2(handle.node), handle: describeComponentHandle2(handle) },
  [
  ...
  handlesByNode2(context).forEach((nodeHandles) => {
    ...
    if (upstreamData) {
      ...
      issues.push(translatedIssue2(
        ...
        {
          component: componentName2(node),
          source: componentName2(upstreamData.node)
        },
        [
...
var checkSignalOutputWithoutConsumer = (context) => context.componentLinkedNets.flatMap((net) => signalRuleDefinitions.flatMap((definition) => {
  ...
  return sources.filter((source) => source.connectedEdges.length > 0).filter((source) => !sinks.some((candidate) => candidate.key !== source.key)).map((source) => translatedIssue2(
    "network-rules",
    "signalOutputWithoutConsumer",
...
var checkDataDirectionWrong = (context) => context.componentLinkedNets.filter((net) => net.classifications.includes("digital_net_type")).flatMap((net) => {
  const dataSources = net.handles.filter((handle) => hasFunction3(handle, "dig_out") && !hasFunction3(handle, "dig_in"));
  const dataSinks = net.handles.filter((handle) => hasFunction3(handle, "dig_in") && !hasFunction3(handle, "dig_out"));
...
var checkComponentHasOnlyOneTerminalConnected = (context) => Array.from(handlesByNode2(context).values()).flatMap((nodeHandles) => {
  ...
  return [translatedIssue2(
    ...
    "warning",
    { component: componentName2(node), handle: describeComponentHandle2(connectedHandles[0]) },
    [
      nodeTarget2(node),
