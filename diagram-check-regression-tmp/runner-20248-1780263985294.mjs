        ...
        "network-rules": {
          ...
          issues: {
            ...
            pinWireCrossSectionDifficult: {
              ...
              recommendation: "Check whether the connection is practical and whether this cross section is really necessary or can be reduced."
            },
            pinTotalCrossSectionDifficult: {
              ...
              recommendation: "Check whether the connection is practical and whether these cross sections are really necessary or can be reduced."
            },
        ...
        "network-rules": {
          ...
          issues: {
            ...
            pinWireCrossSectionDifficult: {
              ...
              recommendation: "Pruefe, ob die Verbindung gut moeglich ist und ob der Querschnitt wirklich notwendig ist oder verkleinert werden kann."
            },
            pinTotalCrossSectionDifficult: {
              ...
              recommendation: "Pruefe, ob die Verbindung gut moeglich ist und ob die Querschnitte wirklich notwendig sind oder verkleinert werden koennen."
            },
            duplicateParallelWire: {
        ...
        "network-rules": {
          ...
          issues: {
            ...
            pinWireCrossSectionDifficult: {
              ...
              recommendation: "\u68C0\u67E5\u6B64\u8FDE\u63A5\u662F\u5426\u5BB9\u6613\u53EF\u9760\u5B8C\u6210\uFF0C\u5E76\u786E\u8BA4\u8BE5\u6A2A\u622A\u9762\u79EF\u662F\u5426\u786E\u6709\u5FC5\u8981\uFF0C\u6216\u662F\u5426\u53EF\u4EE5\u51CF\u5C0F\u3002"
            },
            pinTotalCrossSectionDifficult: {
              title: "\u591A\u6839\u5BFC\u7EBF\u53EF\u80FD\u8F83\u96BE\u4E00\u8D77\u8FDE\u63A5",
              ...
              recommendation: "\u68C0\u67E5\u6B64\u8FDE\u63A5\u662F\u5426\u5BB9\u6613\u53EF\u9760\u5B8C\u6210\uFF0C\u5E76\u786E\u8BA4\u8FD9\u4E9B\u6A2A\u622A\u9762\u79EF\u662F\u5426\u786E\u6709\u5FC5\u8981\uFF0C\u6216\u662F\u5426\u53EF\u4EE5\u51CF\u5C0F\u3002"
            },
            duplicateParallelWire: {
              title: "\u91CD\u590D\u7684\u5E76\u8054\u8FDE\u63A5",
...
var CROSSSECTION_TOLERANCE_MM2 = 1e-6;
...
var netHasPowerClassification = (net) => Boolean(net && (net.classifications.includes("suppl_net_type") || net.classifications.includes("gnd_net_type") || net.classifications.includes("usb_net_type")));
...
var addDirectSourceEdge = (edge, source, edgeNetById, discovery) => {
  const net = edgeNetById.get(edge.id);
  if (!netHasPowerClassification(net) || !net) return;
...
var discoverSources = (context, edgeNetById) => {
  const discovery = {
  ...
  context.nodes.forEach((node) => {
    ...
    sourceHandles.filter((handle) => hasAnyFunction(handle, ["suppl_out", "usb_power_out"])).forEach((handle) => markSupplyConnectorSourceArea(context, handle, source, edgeNetById, discovery));
    sourceHandles.filter((handle) => hasFunction3(handle, "gnd")).forEach((handle) => {
      handle.connectedEdges.forEach((edge) => addDirectSourceEdge(edge, source, edgeNetById, discovery));
    });
  });
  return discovery;
};
...
var checkSourceAndSupplyProtection = (context, settings, edgeNetById, discovery, graph) => {
  const issues = [];
  ...
  discovery.directSourceEdgeIds.forEach((edgeId) => {
    ...
    const net = edge ? edgeNetById.get(edge.id) : void 0;
    const evaluated = edge ? evaluatedWire(edge, settings) : void 0;
    const requiredA = net ? discovery.directSourceNetRequiredA.get(net.id) : void 0;
    if (!edge || !evaluated || !net || requiredA === void 0) return;
...
var checkComponentGroundCrosssections = (context, settings) => {
  ...
  return context.nodes.filter((node) => !isPureConnector(node)).filter((node) => node.data.group !== "led").flatMap((node) => {
    ...
    const supplyEdges = handles.flatMap((handle) => handle.connectedEdges.filter(() => context.getNetByHandle(handle)?.classifications.includes("suppl_net_type")));
    const maxSupplyMm2 = Math.max(
      0,
      ...supplyEdges.map((edge) => edgeCrosssectionMm2(edge, settings)).filter((value) => value !== void 0)
    );
    if (maxSupplyMm2 <= 0) return [];
    return handles.filter((handle) => hasFunction3(handle, "gnd")).flatMap((handle) => handle.connectedEdges.map((edge) => ({ handle, edge }))).filter(({ handle }) => context.getNetByHandle(handle)?.classifications.includes("gnd_net_type")).flatMap(({ handle, edge }) => {
...
var checkWireProtectionRules = (context, settings) => {
  ...
  const sources = discoverSources(context, edgeNetById);
  const fuses = fuseGraph(fuseBoundaries(context));
  return [
    ...checkSourceAndSupplyProtection(context, settings, edgeNetById, sources, fuses),
